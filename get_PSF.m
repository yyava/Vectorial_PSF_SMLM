function [PSF,dPSF] = get_PSF(p,PupilMatrix,dPupilMatrix,Energy_norm)
...
if strcmp(p.dipoleType,'fixed')
    ...
    [PSF,dPSF] = get_PSF_fixed_4pi(p,PupilMatrix,dPupilMatrix,dipoVec,Energy_norm);
else
...
if strcmp(p.dipoleType,'free')
    PSF = PSF_free./Energy_norm;
    dPSF = dPSFdr_free./Energy_norm;
...
%% local functions
function [PSF_fixed,dPSF_fixed] = get_PSF_fixed_4pi(p,PupilMatrix,dPupilMatrix,dipoVec,Energy_norm)
...
% components, so only a third of the number of FTs are needed
    ...
    PSF_fixed = PSF_fixed./Energy_norm;
    dPSF_fixed = dPSF_fixed./Energy_norm;
end
